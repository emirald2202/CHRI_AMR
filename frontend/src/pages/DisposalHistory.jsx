  ...
  return (
    <DashboardLayout>
      <div className="lg:col-span-12 flex flex-col gap-6 animate-in slide-in-from-bottom-4 duration-500 fade-in">
        ...
        <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 p-6 md:p-8 flex flex-col md:flex-row gap-6 items-center justify-between">
            <div>
              <h2 className="text-[1.6rem] font-extrabold text-gray-800 mb-1.5 flex items-center gap-3">
            ...
            <div className="bg-green-50 text-green-700 font-black px-6 py-3.5 rounded-xl text-lg whitespace-nowrap shadow-sm border border-green-100 flex items-center gap-3">
              <ShieldCheck className="w-6 h-6" /> {requests.length} {t('history.safeDisposals', { defaultValue: 'Safe Disposals' })}
            </div>
        </div>

        <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 p-6 md:p-8 min-h-[30rem]">
          <div className="flex flex-col gap-4">
            {requests.length === 0 ? (
              ...
              requests.map(req => (
                <div key={req._id} className="border border-green-100 bg-green-50/30 rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:shadow-md transition-shadow">
                  <div className="flex-1 w-full relative">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                       <h4 className="font-bold text-gray-800 text-[1.05rem] md:text-[1.1rem]">{t('history.verifiedPkg', { defaultValue: 'Verified Safe Package' })}</h4>
                       <button onClick={() => setExpandedReqId(expandedReqId === req._id ? null : req._id)} className="w-fit text-xs font-bold text-green-700 bg-white hover:bg-green-100 border border-green-200 px-4 py-2 rounded-full transition-colors flex items-center justify-center gap-1.5 shadow-sm">
                    ...
                    {expandedReqId === req._id && (
                       <div className="mt-5 bg-white border border-green-200 rounded-2xl p-5 shadow-sm animate-in fade-in zoom-in-95 duration-200 w-full">
                          ...
                          <div className="space-y-3">
                             {(req.verifiedMedicines && req.verifiedMedicines.length > 0 ? req.verifiedMedicines : req.userMedicines || []).map((med, idx) => (
                                <div key={idx} className="flex justify-between items-center bg-gray-50/80 p-3.5 rounded-xl border border-gray-100">
                                   <div>
                                     <p className="text-[0.9rem] font-bold text-gray-800">{med.medicineName || 'Unknown Medicine'}</p>
                                   ...
                                   <div className="text-right">
                                     <span className="text-[0.7rem] font-bold text-gray-600 bg-white px-2.5 py-1.5 rounded-lg border border-gray-200 shadow-sm">{t('history.scannedQty', { defaultValue: 'Scanned Qty' })}: {med.remainingQty}/{med.totalQty}</span>
                  ...
                  <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-gray-200">
                    <span className="px-5 py-2 text-[0.75rem] font-black rounded-xl uppercase tracking-wider bg-green-500 text-white shadow-[0_4px_14px_0_rgba(34,197,94,0.39)]">
                      {t('history.destroyedBadge', { defaultValue: 'Destroyed' })}
                    </span>
                    {req.pointsAwarded && (
                       <span className="text-[0.7rem] font-black text-[#f15700] tracking-wider bg-white px-3 py-1.5 rounded-lg border border-[#f15700]/30 shadow-sm mt-1 whitespace-nowrap">
