...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white dark:bg-slate-800 border-b border-gray-100 dark:border-slate-700 sticky top-0 z-50 shadow-sm">
        ...
        <div className="flex justify-between items-center h-24 md:h-28">
          ...
          <div className="hidden md:flex items-center space-x-2">
            ...
            <button 
              ...
              className="text-green-600 border border-green-600 hover:bg-green-50 dark:bg-emerald-900/30 px-4 py-2 rounded-lg font-bold transition-all flex items-center gap-2"
            >
          ...
          <div className="flex items-center space-x-3 sm:space-x-5">
            ...
            <div className="relative">
              ...
              >
                <div className="text-right hidden sm:block">
                  <div className="text-[0.8rem] font-bold text-gray-800 dark:text-slate-200 leading-tight">{user?.name || 'Guest User'}</div>
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-3 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] border border-gray-100 dark:border-slate-700 py-1.5 z-[1001]">
                  <div className="px-4 py-3 border-b border-gray-50 bg-gray-50 dark:bg-slate-900 rounded-t-xl">
                    <p className="text-sm font-bold text-gray-900 dark:text-slate-200 truncate">{user?.name || 'Guest User'}</p>
                  ...
                  <div className="py-1">
                    ...
                    <Link to="/history" onClick={() => setDropdownOpen(false)} className="flex items-center px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-green-50 dark:bg-emerald-900/30 hover:text-green-700 font-medium transition-colors">
                      <Clock className="w-4 h-4 mr-3 text-gray-400" /> {t('nav.history')}
                    </Link>
                    <Link to="/leaderboard" onClick={() => setDropdownOpen(false)} className="flex items-center px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-green-50 dark:bg-emerald-900/30 hover:text-green-700 font-medium transition-colors">
                      <Award className="w-4 h-4 mr-3 text-gray-400" /> {t('nav.points')}
                    </Link>
                    <button onClick={() => { toggleTheme(); setDropdownOpen(false); }} className="w-full text-left flex items-center px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-green-50 dark:bg-emerald-900/30 hover:text-green-700 font-medium transition-colors">
                      {theme === 'light' ? (
                         <><Moon className="w-4 h-4 mr-3 text-gray-400" /> Dark Mode</>
                      ) : (
                         <><Sun className="w-4 h-4 mr-3 text-gray-400" /> Light Mode</>
                      )}
                    </button>
                  </div>
            ...
            <button 
              className="md:hidden p-1.5 text-gray-500 hover:bg-gray-100 rounded-lg transition-colors border border-transparent hover:border-gray-200 dark:border-gray-700"
      ...
      {/* Sliding Search Bar */}
      {showSearch && (
        <div className="border-t border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-[inset_0_-4px_6px_-6px_rgba(0,0,0,0.1)] pb-4 pt-3 px-4 animate-in slide-in-from-top-2 duration-200">
          <div className="max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-2">
              <div>
                <h3 className="text-gray-800 dark:text-slate-200 font-semibold text-base">{t('nav.findPharmacies')}</h3>
                <p className="text-xs text-gray-500">Find nearby pharmacies on Google Maps</p>
              </div>
            </div>
            <div className="bg-gray-50 dark:bg-slate-900 rounded-lg p-3 border">
              <iframe
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-[9999] flex justify-end">
          ...
          {/* Sidebar */}
          <div className="relative w-64 bg-white dark:bg-slate-800 h-full shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
            <div className="p-4 border-b border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-900">
              <div className="flex justify-between items-start">
                <div>
                  <p className="font-extrabold text-gray-800 dark:text-slate-100 text-sm truncate">{user?.name || 'Guest'}</p>
                ...
                <button 
                  onClick={() => setMobileMenuOpen(false)} 
                  className="p-1.5 text-gray-500 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
            ...
            <div className="px-2 py-3 space-y-1 flex-1 overflow-y-auto">
              ...
              <button 
                ...
                className="w-full text-left px-4 py-3 rounded-xl text-[0.95rem] text-gray-700 dark:text-gray-300 hover:bg-green-50 dark:bg-emerald-900/30 hover:text-green-700 font-bold flex items-center gap-2 transition-colors"
              >
                <Search className="w-[18px] h-[18px] text-green-600" /> {t('nav.findPharmacies')}
              </button>
              <div className="block sm:hidden px-4 pt-3 pb-2 border-t border-gray-100 dark:border-slate-700 mt-2">
                  <LanguageSelector variant="light" />
              </div>
            </div>
          </div>
