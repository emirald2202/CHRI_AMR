  ...
  return (
    <DashboardLayout>
      ...
      <div className="lg:col-span-12 grid grid-cols-1 md:grid-cols-4 gap-6 mb-4">
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-gray-100 dark:border-slate-700 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col justify-center">
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              ...
              <h3 className="text-3xl font-black text-gray-800 dark:text-slate-200">{metrics.pending}</h3>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-gray-100 dark:border-slate-700 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col justify-center">
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              ...
              <h3 className="text-3xl font-black text-gray-800 dark:text-slate-200">{metrics.accepted}</h3>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-gray-100 dark:border-slate-700 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col justify-center">
          <div className="flex items-center gap-4 mb-2">
            <div className="w-12 h-12 bg-green-50 dark:bg-emerald-900/30 rounded-2xl flex items-center justify-center text-green-500">
            ...
            <div>
              ...
              <h3 className="text-3xl font-black text-gray-800 dark:text-slate-200">{metrics.completed}</h3>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-green-600 to-green-700 rounded-3xl p-6 shadow-[0_8px_30px_rgb(5,150,105,0.2)] flex flex-col justify-center text-white">
          <div className="flex items-center gap-4 mb-2">
            <div className="w-12 h-12 bg-white dark:bg-slate-800/20 rounded-2xl flex items-center justify-center text-white">
      ...
      <div className="lg:col-span-12">
        <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 dark:border-slate-700 p-8 min-h-[30rem] flex flex-col">
          
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            <div>
              <h2 className="text-xl md:text-2xl font-black text-gray-800 dark:text-slate-200">{t('pharmacyDashboard.mgmtInbox', {defaultValue: 'Management Inbox'})}</h2>
            ...
            <div className="flex w-full overflow-x-auto custom-scrollbar md:w-auto bg-gray-100 p-1.5 rounded-xl self-start">
              <button 
                ...
                className={`flex-1 md:flex-none whitespace-nowrap px-4 md:px-5 py-2 text-xs md:text-sm font-bold rounded-lg transition-all ${activeTab === 'pending' ? 'bg-white dark:bg-slate-800 shadow-sm text-amber-600' : 'text-gray-500 hover:text-gray-700 dark:text-gray-300'}`}
              >
                {t('pharmacyDashboard.pendTab', {defaultValue: 'Pending'})} ({metrics.pending})
              </button>
              <button 
                onClick={() => setActiveTab('accepted')}
                className={`flex-1 md:flex-none whitespace-nowrap px-4 md:px-5 py-2 text-xs md:text-sm font-bold rounded-lg transition-all ${activeTab === 'accepted' ? 'bg-white dark:bg-slate-800 shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700 dark:text-gray-300'}`}
              >
                {t('pharmacyDashboard.accTab', {defaultValue: 'Accepted'})} ({metrics.accepted})
              </button>
              <button 
                onClick={() => setActiveTab('completed')}
                className={`flex-1 md:flex-none whitespace-nowrap px-4 md:px-5 py-2 text-xs md:text-sm font-bold rounded-lg transition-all ${activeTab === 'completed' ? 'bg-white dark:bg-slate-800 shadow-sm text-green-600' : 'text-gray-500 hover:text-gray-700 dark:text-gray-300'}`}
              >
          ...
          <div className="flex-1 overflow-y-auto pr-2 flex flex-col gap-4">
            {loading ? (
              ...
              filteredRequests.map(req => (
                <div key={req._id} className="border border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-900 rounded-2xl p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-6 hover:bg-white dark:bg-slate-800 hover:shadow-lg transition-all hover:border-green-100 group">
                  
                  {/* Left: Package Details (Double-Blind) */}
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      <h4 className="font-bold text-gray-800 dark:text-slate-200 text-[1.1rem]">{t('history.verifiedPkg', {defaultValue: 'Disposal Package'})}</h4>
                    ...
                    {req.disposalType === 'pickup' && req.pickupAddress && (
                        <p className="text-[0.8rem] text-gray-600 mt-4 flex items-start gap-2 p-3 bg-white dark:bg-slate-800 rounded-xl whitespace-pre-line leading-relaxed border border-gray-200 dark:border-gray-700/60 shadow-sm"><span className="text-lg">🗺️</span> <span className="font-medium">{req.pickupAddress}</span></p>
                    )}
                  </div>
